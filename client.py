...
class RepoPage(tk.Frame):
    ...
    def sendtoServerPath(self, file_path):
        # Create a folder named "serverRepo" if it doesn't exist
        if not os.path.exists("serverRepo"):
            os.makedirs("serverRepo")
        
        # Get the base filename from the file_path
        filename = os.path.basename(file_path)
        
        # Construct the full destination path in the "serverRepo" folder
        destination = os.path.join("serverRepo", filename)
        ...
        try:
            ...
            # Get the destination path by joining the serverRepo folder and the file name
            destination = os.path.join("serverRepo", os.path.basename(file_path))
...
class NetworkPeer(Base):
    def __init__(self, serverhost='localhost', serverport=30000):
        ...
        self.server_info = (serverhost, 40000)
    ...
    def send_register(self):
        ...
        peer_info = {
            'peername': self.name,
            'password': self.password,  # Ensure password is hashed or securely transmitted
            'host': self.serverhost,
            'port': self.serverport
        }
        
        try:
            success = self.client_send(self.server_info,
